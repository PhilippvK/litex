...
_LITEX_TIMESTAMP_RE = re.compile(
    r'^(//|#)\s*Auto-generated by LiteX .* on .*$', re.MULTILINE
)


def _strip_ignored_diffs(text):
    if text is None:
        return None
    # Remove LiteX timestamp lines
    text = _LITEX_TIMESTAMP_RE.sub("", text)
    return text


def write_to_file(filename, contents, force_unix=False, ignore_timestamp=True):
    newline = None
    if force_unix:
        newline = "\n"
    old_contents = None
    if os.path.exists(filename):
        with open(filename, "r", newline=newline) as f:
            old_contents = f.read()
    if ignore_timestamp:
        cmp_old = _strip_ignored_diffs(old_contents)
        cmp_new = _strip_ignored_diffs(contents)
    else:
        cmp_old = old_contents
        cmp_new = contents
